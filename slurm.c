    ...
    if (displaymode != MODE_LARGE)
    {
        ...
        switch(data_type)
        {
			case TYPE_MEGA:
				...
				(stats.rx_over * SNMPMAXBYTES) - stats.rx_bytes_off) / 1048570);
    			strncat(draw, "               ", DRAWLEN-strlen(draw));
				mvprintw(21, 24, "%s", draw);

				snprintf(draw, DRAWLEN-1, "%.3f MB", (stats.tx_bytes +
				(stats.tx_over * SNMPMAXBYTES) - stats.tx_bytes_off) / 1048570);
				strncat(draw, "               ", DRAWLEN-strlen(draw));
				mvprintw(21, 65, "%s", draw);
                break;
			case TYPE_GIGA:
				snprintf(draw, DRAWLEN-1, "%.3f GB", (stats.rx_bytes +
                	(stats.rx_over * SNMPMAXBYTES) - stats.rx_bytes_off)
					/ 1073735680);
                strncat(draw, "               ", DRAWLEN-strlen(draw));
                mvprintw(21, 24, "%s", draw);

                snprintf(draw, DRAWLEN-1, "%.3f GB", (stats.tx_bytes +
                	(stats.tx_over * SNMPMAXBYTES) - stats.tx_bytes_off)
					/ 1073735680);
                strncat(draw, "               ", DRAWLEN-strlen(draw));
                mvprintw(21, 65, "%s", draw);
                break;
...
void led_on(unsigned int who)
{
    ...
    if(who == LED_RX)
    {
		attrset(COLOR_PAIR(PAIR_RX) | 
			((unsigned long)(t.rx_attrib==COL_BOLD)?A_BOLD:A_NORMAL));
        move(14, 37); hline(CHAR, 2);
    }
    else if(who == LED_TX)
    {
		attrset(COLOR_PAIR(PAIR_TX) | 
			((unsigned long) (t.tx_attrib==COL_BOLD)?A_BOLD:A_NORMAL));
        move(14, 41); hline(CHAR, 2);
    }
}
...
int update_stat_large(void)
{
    ...
    {
        rx_maxspeed = 0; rx_overallmax = 0;
        tx_maxspeed = 0; tx_overallmax = 0;
    ...
    if (rx_scalechanged)
    {
        snprintf(draw, DRAWLEN-1, "%.2f KB/s", (float) rx_maxspeed/1024);
		strncat(draw, "               ", DRAWLEN-strlen(draw));
        mvprintw(22, 24, "%s", draw);
    }
    if (tx_scalechanged)
    {
        snprintf(draw, DRAWLEN-1, "%.2f KB/s", (float) tx_maxspeed/1024);
		strncat(draw, "               ", DRAWLEN-strlen(draw));
        mvprintw(22, 65, "%s", draw);
    }
    ...
    if (rx_scalechanged)
    {
        ...
        for (x = GRAPH_WIDTH-1; x >= 0; x--)
        {
            i = (int) ((rx_speedarray[x] / rx_maxspeed) * GRAPHLARGE_HEIGHT);
            for (y = 0; y <= GRAPHLARGE_HEIGHT; y++)
            {   
                if (i > y)
                    rx_graph[x][y] = 1;
    ...
    if (tx_scalechanged)
    {
        ...
        for (x = GRAPH_WIDTH-1; x >= 0; x--)
        {
            i = (int) ((tx_speedarray[x] / tx_maxspeed) * GRAPHLARGE_HEIGHT);
            for (y = 0; y <= GRAPHLARGE_HEIGHT; y++)
            {
                if (i > y)
                    tx_graph[x][y] = 1;
...
int update_stat_split(void)
{
    ...
    if (rx_scalechanged)
    {
        ...
        for (x = GRAPHSPLIT_WIDTH-1; x >= 0; x--)
        {
            i = (int) ((rx_speedarray[x] / rx_maxspeed) * GRAPHSPLIT_HEIGHT);
            for (y = 0; y <= GRAPHSPLIT_HEIGHT; y++)
            {   
                if (i > y)
                    rx_graph[x][y] = 1;
    ...
    if (tx_scalechanged)
    {
        ...
        for (x = GRAPHSPLIT_WIDTH-1; x >= 0; x--)
        {
            i = (int) ((tx_speedarray[x] / tx_maxspeed) * GRAPHSPLIT_HEIGHT);
            for (y = 0; y <= GRAPHSPLIT_HEIGHT; y++)
            {
                if (i > y)
                    tx_graph[x][y] = 1;
...
int update_stat_combined(void)
{
    ...
    if (db_status == DB_STATUS_REINIT || db_status == DB_STATUS_STARTUP)
    {
        ...
        tx_maxspeed = 0; tx_overallmax = 0;
        rx_maxspeed = 0; rx_overallmax = 0;
        comb_maxspeed = 0; comb_overallmax = 0;
        /* fill the graph array with zeros */
        for (x = 0; x < GRAPH_WIDTH; x++)
        {
            for (y = 0; y < GRAPH_HEIGHT; y++)
    ...
    if (sumspeed > comb_maxspeed)
    {
        ...
        if (sumspeed > comb_overallmax)
        {
            ...
            snprintf(draw, DRAWLEN-1, "%.2f KB/s", (float) sumspeed / 1024);
			strncat(draw, "               ", DRAWLEN-strlen(draw));
            mvprintw(19, 65, "%s", draw);
        }
    }
    ...
    if (rx_scalechanged)
    {
        ...
        for (x = GRAPHCOMBINED_WIDTH-1; x >= 0; x--)
        {
            i = (int) ((rx_speedarray[x] / comb_maxspeed) * GRAPHSPLIT_HEIGHT);
            for (y = 0; y <= GRAPHCOMBINED_HEIGHT; y++)
            {
                if (i > y)
                    rx_graph[x][y] = 1;
...
void draw_face(int displaymode, char *hostname)
{
	...
	attrset(COLOR_PAIR(PAIR_TEXT) | 
		((unsigned long)(t.text_attrib==COL_BOLD)?A_BOLD:A_NORMAL));
    for(x = 0; x <= 24; x++)
	{
        for(y = 0; y <= 80; y++)
        {
...
{  
    ...
    /* overruns */
    stats.rx_over = 0; stats.tx_over = 0;
    ...
    if (ifdata.if_speed >= 1000)
        snprintf((char *) ifdata.if_speedstring, sizeof(ifdata.if_speedstring), "%d Mbit/s",
            (int) ifdata.if_speed/1000);
    else if ((ifdata.if_speed == 0) || (ifdata.if_speed ==  ERR_IFACE_NO_SPEED))
   ...
   return 0;
	...
	signal(SIGHUP,  slurm_shutdown); signal(SIGINT,  slurm_shutdown);
	signal(SIGQUIT, slurm_shutdown); signal(SIGKILL, slurm_shutdown);
